...
class AppRequest[B](
    ...
    with AppRequestHeader

/**
  * A request with identity and authenticator traits.
  */
trait AppSecuredRequestHeader[E <: Env] extends AppRequestHeader with SecuredRequestHeader[E]
...
) extends AppRequest(request, messagesApi)
    with AppSecuredRequestHeader[E]

/**
  * A request with optional identity and authenticator traits.
  */
trait AppUserAwareRequestHeader[E <: Env] extends AppRequestHeader with UserAwareRequestHeader[E]
...
class AppUserAwareRequest[E <: Env, B](
    ...
    with AppUserAwareRequestHeader[E]
